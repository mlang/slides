...
from other digital formats to Braille, I found GOODFEEL from
Dancing Dots.
...
is significantly eaiser to implement.
...
* Adapted algorithm from TeX, Donald E. Knuth
  "Breakling Paragraphs into Lines" with modifications
...
The user interface displays LilyPond processed graphical notation along with Braille editor.

Click on a note in the visual display, and the cursor jumps to Braille.

Move the cursor in the Braille editor, and the corresponding notes
will light up.

Ideal for cooperation between sighted and blind.
...
It is free to use, distribute and modify (see LICENSE for details).
